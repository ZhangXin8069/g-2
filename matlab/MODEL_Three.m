...
f=@(x,M,G,F) F^2*((M^4+M^2*G^2)./(24*pi*((x-M^2).^2+M^2*G^2)));
...
% g_exact=G1+2*G2;
% 
% eps=0.001; %%% 真实算出来的相对误差是eps的一半
% f_delta=f_exact+eps*f_exact.*(2*rand(length(f_exact),1)-1);
% g_delta=g_exact+eps*g_exact.*(2*rand(length(g_exact),1)-1);
% G_err=trapz(y,(g_delta-g_exact).^2)/trapz(y,(g_exact).^2);

% figure(4)
% plot(y,g_exact,y,g_delta)
% title(sqrt(G_err))
%%
